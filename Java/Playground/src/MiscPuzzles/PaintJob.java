...
* NOTE: Do not add the MiscPuzzles.ShiftToLeft method to the solution code.
* */
...
//        getBucketCount(0.0, 2.1, 1.5, 7);
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket, int extraBuckets) {
        ...
        if ((Math.signum(width) == -1 || width <= 0 || Math.signum(height) == -1 || height <= 0 || Math.signum(areaPerBucket) == -1 || areaPerBucket <= 0 || Math.signum(extraBuckets) == -1)) {
            bucketsToBuy = -1;
...
//            System.out.println(bucketsToBuy);
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket) {
        ...
        if ((Math.signum(width) == -1 || width <= 0 || Math.signum(height) == -1 || height <= 0 || Math.signum(areaPerBucket) == -1 || areaPerBucket <= 0)) {
            bucketsToBuy = -1;
            return bucketsToBuy;
        } else {
            return bucketsToBuy;
    ...
    public static int getBucketCount(double area, double areaPerBucket) {
        ...
        if (((Math.signum(area) == -1 || area <= 0 || Math.signum(areaPerBucket) == -1 || areaPerBucket <= 0))) {
            bucketsToBuy = -1;
            return (bucketsToBuy);
        } else {
            return (bucketsToBuy);
